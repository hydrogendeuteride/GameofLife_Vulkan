...
void RenderPassManager::init(EngineContext *context)
{
    ...
    // Simple demo compute pass (Game of Life), rendered via ImGui.
    auto golPass = std::make_unique<GameOfLifePass>();
    golPass->init(context);
    addPass(std::move(golPass));
