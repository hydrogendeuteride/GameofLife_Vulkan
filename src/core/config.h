...
// Engine ImGui debug UI (menu bar + debug windows)
// - Default: disabled to keep derived projects minimal.
// - Enable at runtime by setting environment variable `VE_ENGINE_DEBUG_UI=1`.
inline constexpr bool kEnableEngineDebugUIByDefault = false;
inline bool engineDebugUIEnabled()
{
    const char *env = std::getenv("VE_ENGINE_DEBUG_UI");
    if (env && *env)
    {
        return (*env == '1') || (*env == 'T') || (*env == 't') || (*env == 'Y') || (*env == 'y');
    }
    return kEnableEngineDebugUIByDefault;
}
